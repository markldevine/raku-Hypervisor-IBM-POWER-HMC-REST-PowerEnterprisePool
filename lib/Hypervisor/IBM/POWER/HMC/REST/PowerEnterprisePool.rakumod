...
unit    class Hypervisor::IBM::POWER::HMC::REST::PowerEnterprisePool:api<1>:auth<Mark Devine (mark@markdevine.com)>
...
method init () {
    ...
    my $init-start          = now;
    my $fetch-start         = now;
    my $xml-path            = self.config.session-manager.fetch('/rest/api/uom/PowerEnterprisePool');
